...
if (length(args) < 1) {
  stop("Either one or two argument(s) must be supplied. 1) a valid path to an xml file with/without 2) the desired output path.", call.=FALSE)
} else {
  ## extract the argument, the path to file
  path.string = args[1]
  ## args[2] is used later...
}
